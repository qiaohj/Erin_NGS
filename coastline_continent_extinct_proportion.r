...
getResult<-function(p_all, group, TEZ, nb, da, filename, folder){
...
target<-targets[1]
folder<-folders[1]
coastline_groups<-raster("../../Coastline_Groups.tif")
df_result_length<-data.frame()
df_result_lat<-data.frame()
for (folder in folders){
    scs <- sprintf("../../coastline_direction_length_%s/%s", "w_e", folder)
    ...
    for (ttt in scs_items){
      ...
      #plot(length)
      p_all_t<-data.frame(rasterToPoints(coastline_groups))
      colnames(p_all_t)[3]<-"group"
      nb<-"broad"
      da<-"Good"
      print(paste(scs, ttt))
      for (nb in c("medium", "narrow")){
        ...
        for (da in c("Good", "Poor")){
          ...
          item<-getResult(p_all, "group", "ALL", nb, da, filename, folder)
          
          if (nrow(p_all[which(p_all$TEZ==0),])>0){
            item_temp<-getResult(p_all[which(p_all$TEZ==0),], "group", "out", nb, da, filename, folder)
            item<-rbind(item, item_temp)
          }
          
          if (nrow(p_all[which(p_all$TEZ==1),])>0){
            item_temp<-getResult(p_all[which(p_all$TEZ==1),], "group", "in", nb, da, filename, folder)
            item<-rbind(item, item_temp)
          }
...
write.table(df_result_length, "../../Tables/extinct_proportion_continent_removed.csv", row.names=F, sep=",")
