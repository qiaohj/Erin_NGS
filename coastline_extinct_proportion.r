...
write.table(df_result_direction, "../Tables/extinct_proportion_direction_removed.csv", row.names=F, sep=",")
write.table(df_result_lat, "../Tables/extinct_proportion_lat_removed.csv", row.names=F, sep=",")


library(ggplot2)
df_result_direction<-read.table("../Tables/extinct_proportion_direction.csv", head=T, sep=",", stringsAsFactors = F)
head(df_result_direction)
df_result_direction$label<-paste(df_result_direction$ttt, df_result_direction$nb, df_result_direction$da,
                                 df_result_direction$TEZ)
ggplot(df_result_direction[which((df_result_direction$type=="Fake")&
                                   #(df_result_direction$TEZ=="out")&
                                   (df_result_direction$nb=="medium")&
                                   (df_result_direction$da=="Poor")&
                                   (df_result_direction$ttt=="Alex_Eocene_Getech_1")&
                                   (11==11)),])+
  geom_point(aes(x=direction, y=extinct_proportion_overall, color=factor(label)))+
  geom_line(aes(x=direction, y=extinct_proportion_overall, color=factor(label)))

df_result_direction_SE<-summarySE(df_result_direction, "extinct_proportion_overall", c("direction", "TEZ", "type"))
df_result_direction_SE$label<-paste(df_result_direction_SE$TEZ, df_result_direction_SE$type)
df_result_direction_SE$group<-paste(df_result_direction_SE$direction, df_result_direction_SE$TEZ, df_result_direction_SE$type)

ggplot(df_result_direction_SE[which(df_result_direction_SE$TEZ!="in"),])+
  geom_point(aes(x=direction, y=mean, shape=factor(TEZ), color=factor(type)))+
  geom_line(aes(x=direction, y=mean, linetype=factor(TEZ), color=factor(type), group=factor(label)))



head(df_result_lat)
df_result_lat$label<-paste(df_result_lat$ttt, df_result_lat$nb, df_result_lat$da,
                                 df_result_lat$TEZ)
ggplot(df_result_lat[which((df_result_lat$type=="Fake")&
                                   #(df_result_lat$TEZ=="out")&
                                   (df_result_lat$nb=="medium")&
                                   (df_result_lat$da=="Poor")&
                                   (df_result_lat$ttt=="Eocene_1_10deg_Erin_gradient")&
                                   (11==11)),])+
  geom_point(aes(x=y, y=extinct_proportion_overall, color=factor(label)))+
  geom_line(aes(x=y, y=extinct_proportion_overall, color=factor(label)))
