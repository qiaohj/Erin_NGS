  ...
  for (folder in folders){
    ...
    for (ttt in scs_items){
      ...
      for (i in which(p$v==0)){
        pp<-p[i,]
        #for x axis
        pp_left<-p[which((p$x==pp$x)&(p$y==pp$y-1)),]
        if (nrow(pp_left)>0){
          next()
        }
        if (p_v[i, "v"]!=0){
        ...
        while(gap_left>0){
          pp_right<-p[which((p$x==pp$x)&(p$y==pp$y+step)),]
          if (nrow(pp_right)==0){
            gap_left<-gap_left-1
            next()
          }else{
      ...
      dir.create(sprintf("../coastline_direction_length_n_s/%s", folder))
      writeRaster(rr, sprintf("../coastline_direction_length_n_s/%s/%s.tif", folder, ttt), overwrite=T)
