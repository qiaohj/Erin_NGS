...
threshold<-8
df<-df[which(df$length>=threshold),]
for (nb in c("all", "narrow", "medium")){
  for (tez in unique(df$TEZ)){
    for (sc in names(scs)){
      ...
      ggsave(sprintf("../Figures/direction_length_final/%s/%s_%s_with_da.pdf", sc, nb, tez), p, useDingbats=FALSE)
      
      df_item_se<-summarySE(df_item, "extinct_proportion_overall", c("direction", "scenario_abbr"))
      p<-ggplot(df_item_se) + 
        geom_point(aes(x=factor(direction), y=mean, 
                       color=scenario_abbr), 
                   size=3, position=position_dodge(dodge_width))+
        geom_line(aes(x=factor(direction), y=mean, 
                      color=scenario_abbr, group=scenario_abbr),
                  position=position_dodge(dodge_width))
      
      p<-p+ scale_colour_manual(values = colors) +theme_bw()
      #dir.create(sprintf("../Figures/direction_length_final/%s", sc))
      ggsave(sprintf("../Figures/direction_length_final/%s/%s_%s_without_da.pdf", sc, nb, tez), p, useDingbats=FALSE)
